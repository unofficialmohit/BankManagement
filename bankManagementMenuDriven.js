...
function displayAccounts() {
  ...
  if (dataBase.length == 0) {
    prompt("No Account Exists");
    mainmenu();
  }
  3;
  console.log("Account Availables : ");

  for (let key in dataBase) {
    console.log(
      count + " " + dataBase[key].name + "  " + dataBase[key].acNumber
    );
    count++;
  }
...
function createNewUser() {
  ...
  for (let key in dataBase) {
    if (
      dataBase[key].phone == customerPhone ||
      dataBase[key].email == customerEmail
    ) {
...
function oldUser() {
  ...
  console.log(
    "ENTER ACCOUNT NUMBER AND PHONE NUMBER TO ACCESS YOUR ACCOUNT : "
  );
...
function validateLoan(index, type) {
  ...
  if (dataBase[index].loanAlreadyGiven[type]) {
    prompt(
      "You Already have a pending loan due of " +
        dataBase[index].loanTotal[type]
    );
    userOptions(index);
  }
  ...
  dataBase[index].emiCost[type] =
    Math.round((dataBase[index].loanTotal[type] / month) * 100) / 100;
  dataBase[index].balance += value;
  moneyBank -= value;
  ...
  console.log(
    "Total Interest Amount That Bank will Recieve from you (" +
      interest +
      "% PA) : " +
      value * (interest / 100)
  );
...
function validateEmi(index, type) {
  ...
  if (dataBase[index].balance < dataBase[index].emiCost[type]) {
    +console.log(
      "Sorry, Your Account dont have enough funds to repay EMI(" +
        dataBase[index].emiCost[type] +
        "Rupees )"
    );
    console.log(
      "You still have " + dataBase[index].emiCount[type] + " EMI Left to Pay"
    );
  ...
  console.log(
    dataBase[index].emiCost[type] +
      " rupees deducted from your account succesfully"
  );
